...
const MessageItem = ({ message, onClick, isSelected }) => {
  ...
  if (isGroup) {
    avatar =
      message.thumb_group || "../static/media/Logo.af2b2f1b32b135402e38.png";
    nameDisplay = message.name_conversation || "Multi Aura";
  } else {
    const otherUser = message.users.find(
  ...
  return (
    <li
      className={`list-group-item d-flex align-items-center message-item ${
        isSelected ? "selected" : ""
      }`}
