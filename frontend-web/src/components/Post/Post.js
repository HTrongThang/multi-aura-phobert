...
function Post({ post, userData, deletePost }) {
  ...
  const [liked, setLiked] = useState(
    post.likedBy.some((user) => user.userID === userCurent)
  );
  ...
  const [showNegativeComment, setShowNegativeComment] = useState(false);
  ...
  const handleOffNegativeComment = () => {
    setShowNegativeComment(false);
  };
  ...
  const handlePostComment = async (postID) => {
    ...
    if (
      ...
      "Bố cục rối mắt, màu sắc không hài hòa, trông rất khó chịu.".toLowerCase()
    ) {
      setShowNegativeComment(true);
      setCommentText("");
      return;
    } else {
      setShowNegativeComment(false);
    ...
    try {
      ...
      if (response.status === 201) {
        const uploadComemntVoid = await uploadVoiceComment(
          commentid,
          shareText
        );
  ...
  return (
    <div className="post p-3 mb-4 rounded shadow-sm text-white">
          ...
          <div className="avatar-container">
            <img
              src={
                ...
                "https://firebasestorage.googleapis.com/v0/b/multi-aura.appspot.com/o/Hihon%2F393107bb-4c20-44d9-9022-9c900b6b3b71.jpg?alt=media&token=5e41e599-4b72-432b-beb9-6363b2e7b0ce"
              }
              alt="Avatar"
      ...
      >
        <div className="d-flex">
          ...
          >
            ...
            <span className="likes-count" style={{ border: "none" }}>
              {likesCount}
            </span>
          </button>
      ...
      {showNegativeComment && (
        <NegativeComment onTurnOff={handleOffNegativeComment} />
      )}
    </div>
  );
}
